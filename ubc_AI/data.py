...
class dataloader(object):
    ...
    #author:wanghf, splitNoShuffle method is used to split the dataset, the dataset has been shuffled.
    def splitNoShuffle(self, pct=0.95):
        ...
        # cut = int(pct*L)
        cut = 3632
        self.train_target = target[:cut]
        self.train_pfds = pfds[:cut]
        self.test_target = target[cut:]
        self.test_pfds = pfds[cut:]

        self.trainclassifiers = {}

    def split(self, pct=0.9):
        """
    ...
    # Author:wanghf. learning_curve_New method is seted up by wanghf,
    # it mainly output the learning curve by different dataset for ResNet.

    def learning_curve_New(self, classifier):
        self.splitNoShuffle()
        X = self.train_pfds
        y = self.train_target
        Xval = self.test_pfds
        ...
        t_F1 = classifier.score(X[0:v+1], y[0:v+1])
        time_1 = time.time()
        v_F1 = classifier.score(Xval, yval)
        time_diff = time.time() - time_1
        print time_diff
        print("training F1 score = {:.5f}, validation F1 score = {:.5f}".format(t_F1, v_F1))

    # Author:wanghf. learning_curve_Old is used for PICS
    def learning_curve_Old(self, classifier):
        self.split()
        X = self.train_pfds
        y = self.train_target
        Xval = self.test_pfds
