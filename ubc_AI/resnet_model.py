...
class ResNet(object):
    ...
    def _residual(self, x, input_channels, output_channels, stride, activate_before_residual=False):
        ...
        with tf.variable_scope('sub1'):
            # 3*3 conv
            x = self._conv('conv1', x, 3, input_channels, output_channels, stride)#kernel size  7 is not better than kernel size 3.
        ...
        with tf.variable_scope('sub2'):
            ...
            x = self._relu(x, self.hps.relu_leakiness)
            # 3*3 conv step=1
            x = self._conv('conv2', x, 3, output_channels, output_channels, [1, 1, 1, 1])#kernel size 11 is not better than kernel size 3.
